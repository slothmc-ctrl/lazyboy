...
let activeSelect: { close: () => void } | null = null;
...
export function Select(props: SelectProps): TemplateResult {
   ...
   // Create refs
   const triggerRef: Ref<HTMLButtonElement> = createRef();
   ...
   if (isGrouped) {
      (options as SelectGroup[]).forEach((group) => {
         flatOptions.push(...group.options.filter((opt) => !opt.disabled));
      });
   } else {
      flatOptions.push(...(options as SelectOption[]).filter((opt) => !opt.disabled));
   }
   ...
   const updatePosition = () => {
      if (!triggerRef.value || !portalContainer) return;
   ...
   const open = () => {
      if (disabled || state.isOpen) return;
   ...
   const renderPortal = () => {
      if (!portalContainer || !state.isOpen) return;
